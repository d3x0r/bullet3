...
void InitDefaultWorld( bool sloped )
{
...

	{
		...
		btCollisionShape* part = new btBoxShape( btVector3::One );
		{
			int x, y, z;
         for( x = -2; x <= 2; x++ )
				for( y = -2; y <= 2; y++ )
					for( z = -2; z <= 2; z++ )
					{
						init->setOrigin( btVector3::xAxis*x*2 + btVector3::yAxis*y*2 + btVector3::zAxis*z*2 );
                  fallShape2->addChildShape( *init, part );
					}
...
int main( int argc, char** argv )
{
	...
	LoggingFlags = PredictedTransform;
